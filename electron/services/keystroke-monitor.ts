...
function handleKeyDown(event: { keycode: number }) {
  try {
    ...
    console.error('[Big Brother] Keystroke handler error:', err)
  }
}
...
function flushBuffer() {
  ...
  if (text.length >= 3 && onFlush) {
    try {
      ...
      console.error('[Big Brother] Keystroke flush callback error:', err)
    }
  }
  buffer = ''
}
...
export function startKeystrokeMonitor(callback: (text: string) => void) {
  ...
  uIOhook.on('error', (err: any) => {
    console.error('[Big Brother] uIOhook error:', err)
  })

  try {
    uIOhook.start()
    isRunning = true
    console.log('[Big Brother] uIOhook hook started successfully')
  } catch (err) {
    console.error('[Big Brother] uIOhook start failed:', err)
    isRunning = false
  }
}
